...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public DefaultMQProducerImpl(final DefaultMQProducer defaultMQProducer, RPCHook rpcHook) {
        ...
        // 默认的异步的发送者线程池，核心线程数和最大线程数都是当前机器的可用核心数量
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 如果组名不等于 CLIENT_INNER_PRODUCER，则修改当前的实例名称
                // 实例名称为 pid + '#' + nanoTime
                if (!this.defaultMQProducer.getProducerGroup().equals(MixAll.CLIENT_INNER_PRODUCER_GROUP)) {
                    this.defaultMQProducer.changeInstanceNameToPID();
                }

                // 获取或者创建一个 MQClientInstance
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQProducer, rpcHook);

                // 生产者将自己注册到 MQClientInstance 里的 producerTable 里面去，注册不成功则抛出异常
                ...
                // 添加一个 TopicPublishInfo 对象到 topicPublishInfoTable 里面
    ...
    private void checkConfig() throws MQClientException {
        ...
        // 校验组名不能等于 DEFAULT_PRODUCER，因为这个组名是默认使用的
    ...
    private SendResult sendDefaultImpl(
        ...
        // 尝试去寻找 Topic 的发布信息是否存在，如果不存在则创建一个，并从 NameServer 里更新 Topic 的路由信息
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            // 获取消息的发送次数
            // 如果是同步模式，则取 1 加上重试次数，默认为 2，则发送次数为 3，如果是异步模式，则就要发送一次就好了
            ...
            // for 循环执行，如果发送次数超过了总数就跳出循环
            for (; times < timesTotal; times++) {
                // 记录一下上一次发送的 Broker 的名称
                String lastBrokerName = null == mq ? null : mq.getBrokerName();
                // 选择一个 MessageQueue 进行发送
                MessageQueue mqSelected = this.selectOneMessageQueue(topicPublishInfo, lastBrokerName);
                if (mqSelected != null) {
                    // 将这个 MessageQueue 先记录一下，如果此次发送失败了，就需要根据这个 MessageQueue 来选择不同的 broker 来发送
                    mq = mqSelected;
                    // 记录一下 Broker 的名称
                    ...
                    try {
                        ...
                        if (times > 0) {
                            ...
                            // 如果是重试操作，则需要将消息的 topic 根据命名空间重新设置一下
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        // 从本地获取这个 broker 的 master 节点的地址
        String brokerAddr = this.mQClientFactory.findBrokerAddressInPublish(mq.getBrokerName());
        // 如果不存在，则尝试去 NameServer 获取地址
        if (null == brokerAddr) {
        ...
        if (brokerAddr != null) {
            // 判断是否开启了虚拟 IP，如果开启了则通过虚拟 IP 来进行通信
            // 这个 VIP Channel（Virtual IP Channel 使用 10909端口进行通信，作用是提供高可用性和容错能力
            // 当集群中的 Broker 节点发生故障的时候，就可以切换到 VIP Channel 来继续通信
            brokerAddr = MixAll.brokerVIPChannel(this.defaultMQProducer.isSendMessageWithVIPChannel(), brokerAddr);

            byte[] prevBody = msg.getBody();
            try {
                //for MessageBatch,ID has been set in the generating process
                // 如果消息不是批量消息，那么就要给消息设置一个唯一ID
                // 例如：{UNIQ_KEY=7F000001456818B4AAC2607105BF0000}
                if (!(msg instanceof MessageBatch)) {
                    MessageClientIDSetter.setUniqID(msg);
                }

                boolean topicWithNamespace = false;
                // 如果 NameSpace 存在，则设置一下消息的命名空间
                ...
                // 如果单条消息大于 4KB，那么就压缩一下。5000 byte 可以压缩到 107 bytes
                if (this.tryToCompressMessage(msg)) {
                    // 通过位运算的方式，将是否压缩和压缩的算法设置到 sysFlag 字段中
                    sysFlag |= MessageSysFlag.COMPRESSED_FLAG;
                    sysFlag |= compressType.getCompressionFlag();
                    msgBodyCompressed = true;
                }

                // 判断消息是否为事务的预处理消息，如果是则将状态写入 sysFlag 中
                final String tranMsg = msg.getProperty(MessageConst.PROPERTY_TRANSACTION_PREPARED);
                if (Boolean.parseBoolean(tranMsg)) {
                    sysFlag |= MessageSysFlag.TRANSACTION_PREPARED_TYPE;
                }

                // 执行钩子函数，允许用户在发送消息之前执行自定义的逻辑
                ...
                // 创建一个发送消息的请求头，其中包含了发送消息的必要信息，比如说 topic，queueId等等
