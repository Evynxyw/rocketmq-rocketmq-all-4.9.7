...
public class BrokerController {
    ...
    public BrokerController(
        ...
        // 将创建出来的配置对象保存到 BrokerController 中
        ...
        // 设置存储主机的地址，通常是 Broker 的 IP地址和监听端口的组合
        this.setStoreHost(new InetSocketAddress(this.getBrokerConfig().getBrokerIP1(), this.getNettyServerConfig().getListenPort()));

        // broker 快速失败处理组件
        this.brokerFastFailure = new BrokerFastFailure(this);
        // 创建一个 Configuration 对象来管理所有的 Broker 相关的配置信息，包括日志、网络、消息存储等配置。
    ...
    public boolean initialize() throws CloneNotSupportedException {
        // 从磁盘加载配置信息到 RocketMQ 中，比如说：主题的配置信息，消费者的偏移量，订阅组，消费者过滤器。
            ...
            try {
                ...
                this.messageStore =
                    new DefaultMessageStore(this.messageStoreConfig, this.brokerStatsManager, this.messageArrivingListener,
                        this.brokerConfig);
                // 如果开启了 DLeger 功能，则创建一个 DLedgerRoleChangeHandler 来管理 CommitLog
                ...
                //load plugin 加载插件
                // 创建一个消息存储插件上下文，传入相关配置信息
                MessageStorePluginContext context = new MessageStorePluginContext(messageStoreConfig, brokerStatsManager, messageArrivingListener, brokerConfig);
                // 根据传入的上下文对象和当前已存在的消息存储实例构建一个新的消息存储实例出来
                this.messageStore = MessageStoreFactory.build(context, this.messageStore);
                // 将 CommitLogDispatcherCalcBitMap 实例添加到消息存储的分发器的开头，用来处理消息分发。
    ...
    public void start() throws Exception {
        ...
        // 启动 broker 的外部交互 API 服务，比如说注册 broker 到 NameServer，将 broker 从 NameServer 中下线等
        if (this.brokerOuterAPI != null) {
        ...
        // 如果没有启用 DLeger Commit Log，则执行 if 分支内的操作
        if (!messageStoreConfig.isEnableDLegerCommitLog()) {
            // 根据 broker 的角色（主或从）来启动相应的处理器
            startProcessorByHa(messageStoreConfig.getBrokerRole());
            // 处理从节点的数据同步
            handleSlaveSynchronize(messageStoreConfig.getBrokerRole());
            // 注册 Broker 的各种信息到 NameServer
            this.registerBrokerAll(true, false, true);
        }

        // 创建一个定时任务，周期性执行注册 broker 的操作
        // 初始延迟时间为 10 秒，通过 Math 的 min 和 max 方法来保证这个任务的执行间隔在 10 - 60 秒之间
    ...
    public synchronized void registerBrokerAll(final boolean checkOrderConfig, boolean oneway, boolean forceRegister) {
        // 获取当前 broker 实例所有的主题配置信息，并封装成一个 Wrapper 包装对象
        TopicConfigSerializeWrapper topicConfigWrapper = this.getTopicConfigManager().buildTopicConfigSerializeWrapper();

        // 确保 broker 具备写和读的权限，以便可以向 NameServer 注册 topic。
        if (!PermName.isWriteable(this.getBrokerConfig().getBrokerPermission())
            || !PermName.isReadable(this.getBrokerConfig().getBrokerPermission())) {
        ...
        // 判断是否需要进行注册，如果设定了强制注册，或者通过 needRegister 判断这个 broker 需要注册，那么就注册
        // broker 启动后定时任务调度到这个方法的时候， forceRegister 为 true，所以需要强制注册
    ...
    private void doRegisterBrokerAll(boolean checkOrderConfig, boolean oneway,
        ...
        // 此处通过 BrokerOuterAPI 和外部的所有 NameServer 进行网络交互，完成 Broker 的注册操作
        ...
        if (registerBrokerResultList.size() > 0) {
            ...
            if (registerBrokerResult != null) {
                // 如果 Broker 需要定期更新主 Master HA Server 地址，并且注册结果中包含了 HA Server 地址，
                // 则更新消息存储模块的 HA Master 地址为注册结果中的 HA Server 地址
                if (this.updateMasterHAServerAddrPeriodically && registerBrokerResult.getHaServerAddr() != null) {
                    this.messageStore.updateHaMasterAddress(registerBrokerResult.getHaServerAddr());
                }

                // 将注册结果中的 Master 地址设置给从节点同步器
                this.slaveSynchronize.setMasterAddr(registerBrokerResult.getMasterAddr());

                // 检查顺序消息的配置，更新顺序消息的配置信息
                if (checkOrderConfig) {
