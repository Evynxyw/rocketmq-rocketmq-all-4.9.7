...
public class CommitLog {
    ...
    public CommitLog(final DefaultMessageStore defaultMessageStore) {
        ...
        // 判断 broker.conf 中配置的 FlushDiskType 刷盘策略是同步还是异步
...
//        int queueId msg.getQueueId();
    ...
    public CompletableFuture<PutMessageStatus> submitFlushRequest(AppendMessageResult result, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) {
                // 同步刷盘，并且需要等存储完成后返回
                // 创建一个同步刷盘的请求对象，下一个的偏移量为当前已经写入的偏移量加上当前消息的大小
                GroupCommitRequest request = new GroupCommitRequest(result.getWroteOffset() + result.getWroteBytes(),
                        this.defaultMessageStore.getMessageStoreConfig().getSyncFlushTimeout());
                // 将同步刷盘的请求添加到刷盘的观察者中
                flushDiskWatcher.add(request);
                // 将同步刷盘的请求添加到同步刷盘服务内部，并调用一个 wakeup 方法唤醒同步刷盘的线程
                service.putRequest(request);
                // 返回一个 CompletableFuture 对象，没有填充结果
                return request.future();
            } else {
                // 同步刷盘，但是不需要等刷盘的结果，直接唤醒同步刷盘的线程，然后直接返回 PUT_OK 的状态
                service.wakeup();
                return CompletableFuture.completedFuture(PutMessageStatus.PUT_OK);
            }
        }
        // Asynchronous flush 异步刷盘逻辑
        else {
            // 判断是否启动了堆外内存
            if (!this.defaultMessageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
                // 没有启动堆外缓存，那么唤醒异步刷盘服务 FlushRealTimeService
                flushCommitLogService.wakeup();
            } else  {
                // 启动了堆外缓存，则唤醒异步转存服务 CommitRealTimeService
                commitLogService.wakeup();
            }
            // 异步刷盘，不需要等刷盘的结果，直接返回 PUT_OK 的状态
            return CompletableFuture.completedFuture(PutMessageStatus.PUT_OK);
        }
    }
     ...
     * 异步转存服务
     *
     * 写数据的时候需要将数据先写到一个临时的堆外内存 writeBuffer 里，然后再把这个消息写到一个 FileChannel 里面，
     * 接着因为这个 CommitLog 文件使用了内存映射的 mmap ，所以从对应的 MappedByteBuffer 里面就可以拿到 FileChannel 里面的数据。
     *
     * 为什么要弄一个这样的机制呢？
     * 是因为高并发的场景下，写入操作都在 PageCache 上执行会使磁盘压力过大，导致毛刺现象。
     * 引入一个堆外缓存，就可以避免压力全部积压在磁盘上，但是这样也有一定的风险，比如说：延迟、一致性降低等。
     */
    class CommitRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                // 获取刷盘的最大时间间隔，默认 200 毫秒，超过了 200 毫秒则不需要根据最少页数的条件来执行了
                int commitDataThoroughInterval =
                    CommitLog.this.defaultMessageStore.getMessageStoreConfig().getCommitCommitLogThoroughInterval();

                // 判断有没有超过 200 毫秒，如果超过了直接将页数置为 0
                ...
                try {
                    // 执行 commit ,将数据提交到 writeBuffer 堆外缓存中，并写入到一个 FileChannel 通道里面
    ...
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            // 自旋执行
            while (!this.isStopped()) {
                // 获取到定时刷盘的配置，默认 true，定时刷盘
                boolean flushCommitLogTimed = CommitLog.this.defaultMessageStore.getMessageStoreConfig().isFlushCommitLogTimed();
                // 获取刷盘的时间间隔，默认 500 毫秒
                int interval = CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushIntervalCommitLog();
                // 获取刷盘的最少页数，默认 4 页，一页 4KB，也就是要 16KB 才刷盘
                int flushPhysicQueueLeastPages = CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushCommitLogLeastPages();

                // 获取刷盘的时间间隔，默认 10 秒，如果距离上一次刷盘的时间超过 10 秒，不管页数的条件，总是会刷盘
                ...
                // 判断当前时间有没有超过 flushPhysicQueueThoroughInterval 这个时间间隔，也就是有没有超过 10 秒
                if (currentTimeMillis >= (this.lastFlushTimestamp + flushPhysicQueueThoroughInterval)) {
                    // 更新最后的刷新时间
                    this.lastFlushTimestamp = currentTimeMillis;
                    // 更新刷盘页数为 0，意思是不用关心上面配置的 4 页的参数
                    flushPhysicQueueLeastPages = 0;
                    // 是否需要打印刷盘的进度，每 10 次打印一次
                    printFlushProgress = (printTimes++ % 10) == 0;
                }

                try {
                    // 判断是否需要定时执行刷盘操作，如果需要则定时休眠，默认 500 毫秒
                    if (flushCommitLogTimed) {
                        Thread.sleep(interval);
                    } else {
                        // 如果不是定时刷盘，则直接调用 waitForRunning 来进入等待状态，等待 wakeup 方法调用来唤醒执行刷盘
                        this.waitForRunning(interval);
                    }

                    // 打印操作，不过 printFlushProgress 里面的打印操作已经被注释了
                    ...
                    if (storeTimestamp > 0) {
                        // 将这个时间写入到 PhysicMsgTimestamp，便于重启的数据恢复
            ...
            // 自旋判断中 stopped 状态修改为 true 时，说明正在关闭服务了
            // 需要再次执行一下刷盘的操作，如果失败了，会再重试 9 次
            boolean result = false;
            for (int i = 0; i < RETRY_TIMES_OVER && !result; i++) {
    ...
    /**
     * GroupCommit Service
     * 同步刷盘服务，本质上是一个 Runnable，通过线程将此服务运行起来
     */
    class GroupCommitService extends FlushCommitLogService {

        // 用来写入刷盘请求的集合，执行 putRequest 方法写入
        private volatile LinkedList<GroupCommitRequest> requestsWrite = new LinkedList<GroupCommitRequest>();
        // 用来读取刷盘请求的集合，执行 doCommit 方法读取
        // 这种读写分离的方式，可以让 putRequest 和 doCommit 同时进行，不会存在锁竞争的问题。
        // 以前的版本，doCommit 方法是加锁了的，性能没有这么高。
        private volatile LinkedList<GroupCommitRequest> requestsRead = new LinkedList<GroupCommitRequest>();

        // 锁
        private final PutMessageSpinLock lock = new PutMessageSpinLock();

        // 将一个同步请求添加到 requestsWrite 里面，并且唤醒当前线程
        ...
        // requestsWrite 和 requestsRead 交换
        ...
        private void doCommit() {
            ...
            if (!this.requestsRead.isEmpty()) {
                ...
                for (GroupCommitRequest req : this.requestsRead) {
                    ...
                    // two times the flush
                    // 一个消息可能会因为太大然后会存在下一个文件中，所以一次请求肯呢个会存在最大两次刷盘

                    // FlushedWhere 是当前映射文件已经刷新到哪里了  nextOffset 则是下一个请求消息的开头，也就是当前消息的结尾
                    // 如果已经刷新的位置已经大于等于了这个消息的结尾，那就表示这个消息已经刷完了
                    boolean flushOK = CommitLog.this.mappedFileQueue.getFlushedWhere() >= req.getNextOffset();

                    // for 循环两次进行刷盘，需要保证目前的消息是没有刷盘的
                    for (int i = 0; i < 2 && !flushOK; i++) {
                        // 进行刷盘操作，所有消息都会刷盘
                        CommitLog.this.mappedFileQueue.flush(0);
                        // 判断刷完之后的已刷新位置是不是大于等于消息的结尾位置，大于等于的话，继续再循环一次
                        flushOK = CommitLog.this.mappedFileQueue.getFlushedWhere() >= req.getNextOffset();
                    }

                    // 将结果写入 CompletableFuture 中
                ...
                long storeTimestamp = CommitLog.this.mappedFileQueue.getStoreTimestamp();
                if (storeTimestamp > 0) {
                    // 将时间戳写到 PhysicMsgTimestamp 里面，用来做重启的数据恢复
                    CommitLog.this.defaultMessageStore.getStoreCheckpoint().setPhysicMsgTimestamp(storeTimestamp);
                }

                // 将度队列重置为空，当下一次交换队列的时候，写队列置为空
                this.requestsRead = new LinkedList<>();
            } else {
                // Because of individual messages is set to not sync flush, it
                // will come to this process
                // 有些消息的配置是同步刷盘但是不等待，因此这里直接刷盘，不需要唤醒线程
                CommitLog.this.mappedFileQueue.flush(0);
            }
        }
        ...
        public void run() {
            ...
            // 当 stopped 状态修改为停止状态之后，将会休眠 10 毫秒，等所有的请求都到达后将这一批消息请求进行刷盘操作。
            try {
                Thread.sleep(10);
