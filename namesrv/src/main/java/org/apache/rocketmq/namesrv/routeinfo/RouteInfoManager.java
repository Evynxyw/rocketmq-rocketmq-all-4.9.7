...
public class RouteInfoManager {
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                // 通过 clusterName 获取或创建一个 HashSet，其中保存这个集群的所有的 broker 的名称
                Set<String> brokerNames = this.clusterAddrTable.computeIfAbsent(clusterName, k -> new HashSet<>());
                // 将当前注册过来的 broker 的名称保存进去，第一次注册请求发送过来，则是做新增
                // 第二次注册请求发送过来就是做心跳了，因为是 Set ，所以会去重，这里不受影响
                brokerNames.add(brokerName);

                boolean registerFirst = false;

                // 通过 broker 的名称拿到他的相关数据
                BrokerData brokerData = this.brokerAddrTable.get(brokerName);
                // 如果 brokerAddrTable 里面不存在这个 broker 的数据，那么就表示这个是第一次访问，是注册操作
                if (null == brokerData) {
                    // 将第一次注册的标记设置为 true
                    registerFirst = true;
                    // 创建一个新的 broker 数据对象，然后保存到 brokerAddrTable 里面
                ...
                // 将 broker 注册到 brokerData 中去，并将旧地址返回
                String oldAddr = brokerData.getBrokerAddrs().put(brokerId, brokerAddr);
                // 如果 brokerId 是 0，也就是 Master 节点的话，打印日志
                ...
                // 如果主题配置的包装器不为空的话，并且要注册的 broker 是主节点
                ...
                /**
                 * 将 broker 的地址作为 key， BrokerLiveInfo 作为 value 存储到 brokerLiveTable 的 Map 里面去
                 * 这个就是做为心跳检测的一个集合，BrokerLiveInfo 里面会存储当前时间的时间戳
                 *
                 * scanNotActiveBroker() 方法中就会去通过这个参数来判断 Broker 是否活跃
    ...
    public int scanNotActiveBroker() {
        int removeCount = 0;
        // 遍历 brokerLiveTable 这个集合
        Iterator<Entry<String, BrokerLiveInfo>> it = this.brokerLiveTable.entrySet().iterator();
        while (it.hasNext()) {
            Entry<String, BrokerLiveInfo> next = it.next();
            // 获取到 broker 的最后更新时间戳
            long last = next.getValue().getLastUpdateTimestamp();
            // 将最后更新时间加上过期时间，判断是否小于当前时间，小于则说明过期了,超过了 120 秒
            if ((last + BROKER_CHANNEL_EXPIRED_TIME) < System.currentTimeMillis()) {
                // 过期则关闭 Channel 连接，brokerLiveTable 里面移除这个 broker，打印警告信息
