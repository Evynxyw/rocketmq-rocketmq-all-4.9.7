...
public class DefaultRequestProcessor extends AsyncNettyRequestProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        switch (request.getCode()) {
            ...
            case RequestCode.REGISTER_BROKER:
                // 从 request 中获取到 broker 的版本号
                Version brokerVersion = MQVersion.value2Version(request.getVersion());
                // 判断 broker 的版本是否大于等于 3.0.11，若是则走 registerBrokerWithFilterServer，否则走 registerBroker
    ...
    public RemotingCommand registerBrokerWithFilterServer(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        // 从 request 请求对象中获取到请求头
        final RegisterBrokerRequestHeader requestHeader =
            (RegisterBrokerRequestHeader) request.decodeCommandCustomHeader(RegisterBrokerRequestHeader.class);

        // 通过 CRC32 循环冗余校验码来检测数据传输是否存在错误，如果存在错误则返回一个异常实体
        ...
        if (request.getBody() != null) {
            try {
                // 存在则进行解码，将实体保存到 registerBrokerBody 中
        ...
        // 调用 RouteInfoManager 的 registerBroker 方法注册 Broker，传递集群名称、
        // Broker 地址、Broker 名称、Broker ID、高可用服务器地址、主题配置序列化包装器、过滤服务器列表和通道（Channel）
        ...
        // 设置响应头部的 HA 服务器地址和主服务器地址
        responseHeader.setHaServerAddr(result.getHaServerAddr());
        responseHeader.setMasterAddr(result.getMasterAddr());

        // 获取命名空间为 NamesrvUtil.NAMESPACE_ORDER_TOPIC_CONFIG 的键值对列表，并将其设置为响应的主体。
        byte[] jsonValue = this.namesrvController.getKvConfigManager().getKVListByNamespace(NamesrvUtil.NAMESPACE_ORDER_TOPIC_CONFIG);
        response.setBody(jsonValue);

        // 设置响应码为 ResponseCode.SUCCESS，备注为空，然后返回响应
        response.setCode(ResponseCode.SUCCESS);
        response.setRemark(null);
        return response;
    }
