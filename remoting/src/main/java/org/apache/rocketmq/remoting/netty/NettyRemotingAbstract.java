...
public abstract class NettyRemotingAbstract {
    ...
    public RemotingCommand invokeSyncImpl(final Channel channel, final RemotingCommand request,
        ...
        try {
            // 创建一个 ResponseFuture，用来存储后续 Channel 的返回实体
            final ResponseFuture responseFuture = new ResponseFuture(channel, opaque, timeoutMillis, null, null);
            this.responseTable.put(opaque, responseFuture);
            final SocketAddress addr = channel.remoteAddress();

            // 给 Channel 中发送 request 请求实体，并且添加一个监听器来处理操作完成的事件
            // 发送成功，则将 SendRequestOK 状态设置为 true，否则为 false
            channel.writeAndFlush(request).addListener(new ChannelFutureListener() {
                ...
                public void operationComplete(ChannelFuture f) throws Exception {
                    ...
                    // 失败状态下，从 responseTable 中移除这个 responseFuture
                    responseTable.remove(opaque);
                    // 将错误原因写入 responseFuture 的 cause 字段中
            ...
            if (null == responseCommand) {
                if (responseFuture.isSendRequestOK()) {
                    // 响应结果为 null，并且 responseFuture 中的状态是 OK 的，那么表示发送成功了，但是超时了
                    throw new RemotingTimeoutException(RemotingHelper.parseSocketAddressAddr(addr), timeoutMillis,
                        responseFuture.getCause());
                } else {
                    // 如果发送状态是不成功的，则抛出 RemotingSendRequestException 异常
                    throw new RemotingSendRequestException(RemotingHelper.parseSocketAddressAddr(addr), responseFuture.getCause());
                }
            }
            // 发送成功，直接返回 responseCommand
